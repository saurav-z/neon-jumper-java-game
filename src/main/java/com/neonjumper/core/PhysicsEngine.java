...
public class PhysicsEngine {
    ...
    public void update(List<GameObject> objects, double deltaTime) {
        ...
        for (GameObject obj : objects) {
            ...
            // Apply Gravity
            Vector2D newVel = obj.getVelocity().add(new Vector2D(0, GRAVITY * deltaTime));
    ...
    private void resolveCollisions(GameObject obj, List<GameObject> objects, boolean checkX) {
        ...
        for (GameObject other : objects) {
            ...
            if (myBounds.intersects(other.getBounds())) {
                ...
                if (checkX) {
                    if (other.getType() == GameObject.Type.WALL) {
                        if (vel.x() > 0) {
                    ...
                    if (other.getType() == GameObject.Type.WALL) {
                        if (vel.y() > 0) {
                            ...
                            if (obj instanceof com.neonjumper.entity.Player) {
                                ((com.neonjumper.entity.Player) obj).setOnGround(true);
                            }
                        } else if (vel.y() < 0) {
                ...
                if (obj instanceof com.neonjumper.entity.Player player) {
                    if (other.getType() == GameObject.Type.SPIKE && !player.isInvincible()) {
                        player.takeDamage();
                        camera.shake(10, 0.4);
